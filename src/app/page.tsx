...
import { getDestinations, getHotels, getCategories } from '@/lib/directus';
...
export default async function HomePage() {
  // Use mock data for now - later can be replaced with API calls
  const featuredHotel = {
    tag: "New to the Club",
    name: "the cōmodo",
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {['Culinary', 'Spa', 'City', 'Beach', 'Adults Only'].map((category, index) => (
              <Link 
                key={index}
                href={`/hotels/category/${category.toLowerCase().replace(' ', '-')}`}
                className="group block relative h-[250px] rounded-xl overflow-hidden"
              >
                <Image 
                  src={`/images/category-${category.toLowerCase().replace(' ', '-')}.jpg`} 
                  alt={category}
                  fill
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="text-xl font-brooklyn text-white">{category}</h3>
                </div>
              </Link>
            ))}
          </div>
