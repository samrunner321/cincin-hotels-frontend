...
function getImageUrl(imageValue) {
  // If it's a UUID (from Directus), construct the full URL
  ...
  return imageValue || '/images/hotels/hotel-1.jpg';
}
...
export default function DetailHeroBanner({ 
  ...
  return (
    <section className="flex flex-col md:flex-row h-[66vh] w-full relative">
      {/* Hintergrund-Container der vollen Breite */}
      <div className="absolute inset-0 flex w-full h-full">
        {/* Linke Seite - Bild (55%) */}
        <div className="relative w-[55%] h-full">
          <Image
            src={getImageUrl(backgroundImage) || getHotelImage(slug, '/images/hotels/hotel-1.jpg')}
            alt={hotelName}
            fill
            priority
      ...
      <div className="relative flex flex-col md:flex-row w-full h-full max-w-[1814px] mx-auto">
        ...
        <div className="w-full md:w-[45%] h-1/3 md:h-full flex flex-col justify-end p-8 md:p-12 lg:p-16">
          <div className="max-w-md lg:max-w-lg">
            ...
            <p className="text-white/80 mb-8">{displayDescription}</p>
            <a 
              href="#overview" 
